...
const KEY_SHIFT = "[img]res://resources/images/interface/Keyboard & Mouse/Default/keyboard_shift_icon.png[/img]"
const KEY_RETURN = "[img]res://resources/images/interface/Keyboard & Mouse/Default/keyboard_return.png[/img]"
const KEY_SPACE = "[img]res://resources/images/interface/Keyboard & Mouse/Default/keyboard_space_icon.png[/img]"
const KEY_ARROWS = "[img]res://resources/images/interface/Keyboard & Mouse/Default/keyboard_arrows.png[/img]"

# TODO: localize text
var STEPS_TEXTS = [
	"Bienvenue dans Decorporate ! Vous pouvez vous déplacer avec %s et sauter avec %s.",
	"Vous pouvez utiliser votre attaque normale avec %s, et votre attaque spéciale avec %s.",
...
const CONTROL_ICONS = [
	[[KEY_ARROWS, KEY_SPACE], [XBOX_STICK_L, XBOX_A]],
	[[KEY_RETURN, KEY_SHIFT], [XBOX_X, XBOX_B]],
]
...
var player_id: int = 0
var ennemy_id: int = 0
var control_type = ControlTypes.KEYBOARD
...
func get_step_text(n):
	return STEPS_TEXTS[n] % CONTROL_ICONS[n][control_type] if n <= 1 else STEPS_TEXTS[n]
...
func show_step_text(step: int):
	...
	tutorial_text.text = get_step_text(step)
...
func _ready() -> void:
	if len(Input.get_connected_joypads()) >= 1:
		control_type = ControlTypes.CONTROLLER
